...
<div className="w-[100%] lg:max-w-2xl no-scrollbar overflow-x-auto px-4 min-h-[140px]">
  ...
  <div
    key={i}
    className="flex flex-col items-center mx-2 relative overflow-hidden"
  >
    <div className="h-44 w-30 bg-gray-300 rounded-lg overflow-hidden">
    ...
    <div className="flex gap-4 py-10">
      ...
      <div
        ...
        className="
          ...
          min-w-24 max-w-24 max-h-36      /* 📱 mobile (slightly taller) */
          sm:min-w-30 sm:max-w-30 sm:max-h-44  /* 💻 larger screens */
          aspect-[3/4]
          cursor-pointer hover:shadow-lg transition-all duration-200
          border-2 border-dashed border-[var(--input-border)]
          flex items-center justify-center
        "
        style={{ background: "var(--hover-gradient)" }}
      >
        <div className="h-full flex flex-col items-center justify-center p-3 sm:p-4">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-[var(--primary)] rounded-full flex items-center justify-center mb-2 sm:mb-3">
            <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-[var(--text-main)]" />
          </div>
      ...
      {fetchError && (
        <div className="flex flex-col items-center justify-center w-full py-10 text-slate-500 font-medium">
          <XCircle className="w-6 h-6 sm:w-8 sm:h-8 mb-1 sm:mb-2" />
      ...
      {!fetchError && noStories && (
        <div className="flex flex-col items-center justify-center w-full py-10 text-slate-500 font-medium">
          <Inbox className="w-6 h-6 sm:w-8 sm:h-8 mb-1 sm:mb-2" />
          No stories available
          <div onClick={handleAddStoryClick} className="btn mt-2 sm:mt-3">create one now</div>
        </div>
      )}
        ...
        return (
          <div
            ...
            className="
              ...
              min-w-24 max-w-24 max-h-36      /* 📱 mobile (taller) */
              sm:min-w-33 sm:max-w-33 sm:max-h-44 /* 💻 larger screens */
              cursor-pointer transition-all duration-200
              active:scale-95 hover:shadow-lg
              story-gradient overflow-hidden
            "
          >
            {/* Background media */}
            {isImage && (
            ...
            <div className="absolute top-1 left-1 z-10">
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full ring ring-gray-100 shadow overflow-hidden flex items-center justify-center">
                <img
            ...
            {(story.content?.trim() || story.title?.trim()) && (
              <p className="absolute top-16 sm:top-18 left-2 sm:left-3 z-10 text-[var(--text-main)] text-xs sm:text-sm truncate max-w-28 sm:max-w-24">
                {story.content?.trim() || story.title?.trim()}
              </p>
            )}

            {/* Timestamp */}
            <p className="text-[var(--text-main)] absolute bottom-1 right-1 z-10 text-[10px] sm:text-xs">
